...
export const ContactTextArea: React.FC<propTypes> = ({ value, onChange }) => {
  ...
  const moveLabel = useSpring({
    ...
    background: focused ? "linear-gradient(180deg, #1a1a1a 65%, #262626 35%)" : "linear-gradient(180deg, #262626 50%, #262626 50%)",
    config: (key) => {
      if (key === "background") {
  ...
  return (
    <div className="relative h-48 w-full rounded-sm bg-gradient-to-r from-primary via-primary to-secondary p-[0.1rem]">
      <animated.label style={{ ...moveLabel }} htmlFor="message" className="text-textPlaceholder pointer-events-none absolute inset-x-0 top-0 z-30 ml-1.5 w-fit rounded-sm px-1 text-lg">
        Message
      </animated.label>
      <textarea
