...
export const PortfolioItem: React.FC<propTypes> = ({ object }) => {
  return (
    <div className="grid grid-cols-2 xl:gap-8 xl:pl-10 2xl:gap-10">
      ...
      <div className="mx-2 flex w-11/12 flex-col items-center text-white">
        <div className="relative w-fit self-start">
          <div className="relative z-20 xl:pb-2 xl:text-3xl 2xl:pb-4 2xl:text-4xl">{object.title}</div>
        </div>
        <div className="flex self-start xl:pb-2 2xl:pb-3">
          <div className="mr-4 text-tertiary xl:text-lg 2xl:text-xl">Made&nbsp;with:</div>
          <div className="flex flex-wrap xl:w-[28rem] 2xl:w-[40rem]">
            {object.tech.map((tech, key, object) => (
              <PortfolioTechnology tech={tech} key={key} slash={object.length - 1 !== key} />
            ))}
          </div>
        </div>
        <div className="w-full xl:text-lg xl:leading-8 2xl:text-xl 2xl:leading-10">{object.description}</div>
